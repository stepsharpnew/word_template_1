        ...
        <v-hover v-slot="{ hover }">
          ...
          >
            ...
            <div class="d-flex flex-column align-end">
              ...
              <div
                class="d-flex justify-space-around align-center mt-6 flex-column"
              >
                <div class="d-flex align-center">
                  <v-icon v-if="parsing" class="mr-2" spin
                    >mdi-progress-clock</v-icon
                  >
                  <v-icon v-else-if="parsedOk" color="success" class="mr-2"
                    >mdi-check-circle</v-icon
                  >
                  <div class="text-caption">
                    <span v-if="parsing">Парсинг файла...</span>
                    <span v-else-if="parsedOk"
                      >Готов к отправке (объект сформирован)</span
                    >
                  </div>
                </div>
                ...
                <v-hover v-slot="{ hover: btnHover }">
                  <v-btn
                    ...
                    :disabled="
                      !parsedOk || sending || !templateSelected || !reasons
                    "
                    color="primary"
                    dark
                    @click="handleProcess"
  ...
  computed: {
    previewJson() {
      return this.parsedObject
        ? JSON.stringify(this.parsedObject, null, 2)
        : "";
    },
  },
  methods: {
    ...
    onFilesChange(newVal) {
      ...
      let files =
        newVal && newVal.target && newVal.target.files
